...
let ConfigName: string = process.argv[2];
let Config = PackHelperConfig.Mods[ConfigName];
let UPlugin = JSON.parse(FS.readFileSync(path.join(GeneralConfig.GameModFolder, Config.MainMod, `${Config.MainMod}.uplugin`)).toString()) as UPlugin;
if (!Config) {
