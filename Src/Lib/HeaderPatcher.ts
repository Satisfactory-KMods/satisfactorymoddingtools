		...
		for (const File of ScannedDir) {
			...
			if (Stats.isDirectory()) {
				...
				for (const OverwriteData of this.Config.Includer) {
					if (OverwriteData.ClassFileName === File) {
						...
						for (let Idx = 0; Idx < Lines.length; ++Idx) {
							...
							if (
								Lines[Idx].includes('include') &&
								Lines[Idx].includes(OverwriteData.After) &&
								!Content.includes(IncludeString) &&
								!Added
							) {
								Idx++;
				...
				for (const OverwriteData of this.Config.FunctionOverwrite) {
					if (OverwriteData.ClassFileName === File) {
						let FunctionName = OverwriteData.Template.replaceAll('{Class}', OverwriteData.Class).replaceAll(
							'{Function}',
