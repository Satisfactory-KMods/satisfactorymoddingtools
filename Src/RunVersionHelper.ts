...
import { GeneralConfig, VersionControlConfig } from './Config/ToolConfig';
import { FicsitApiHelper } from './Lib/FicsitApiHelper';
import { SemVersionHolder, SemVersionUpgradeHolder, UPlugin } from './Types/FicsitApiHelper';
...
ApiHelper.StartQuery().then(async () => {
	for (const Data of ApiHelper.GetApiData()) {
		...
		if (Allowed) {
			let UpgradeInfo: SemVersionUpgradeHolder =
				VersionControlConfig.ModVersionOverwrite[Data.mod_reference] !== undefined
					? VersionControlConfig.ModVersionOverwrite[Data.mod_reference]
					: {
							version_Small: true
					  };
